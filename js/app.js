...
class DashboardApp {
  ...
  exportPipeline(format) {
    ...
    if (format === 'excel') {
      ...
      const isOverviewActive = document.getElementById('page-overview') && document.getElementById('page-overview').classList.contains('active');
      if (isOverviewActive) {
        this.exportDashboardHTML('overview');
        return;
      }
  ...
  exportAwarded(format) {
    ...
    if (format === 'excel') {
      ...
      const isPortfolioActive = document.getElementById('page-portfolio') && document.getElementById('page-portfolio').classList.contains('active');
      if (isPortfolioActive) {
        this.exportDashboardHTML('portfolio');
        return;
      }
...
<body>
  ...
  verifyDatabaseAccess(event, url) {
    event.preventDefault();
    const password = prompt("กรุณาใส่รหัสผ่านเพื่อเข้าใช้งาน Database:");
    if (password === '22052536') {
  ...
  exportDashboardHTML(type) {
    ...
    const subtitle = isPortfolio ? 'Summary of renewable energy projects with active construction contracts' : 'Dashboard and metrics for pipeline projects in development stage';
    const projects = isPortfolio ? this.getFilteredAwardedProjects() : this.getFilteredPipelineProjects();
    
    // Get table HTML
    const tableBodyId = isPortfolio ? 'portfolio-region-table-body' : 'overview-region-table-body';
    const tableBodyEl = document.getElementById(tableBodyId);
    const tableHtml = tableBodyEl ? tableBodyEl.innerHTML : '';

    // Get system details HTML
    const detailsId = isPortfolio ? 'portfolio-system-details' : 'overview-system-details';
    ...
    // Prepare map markers
    const markersData = projects.map(p => {
      return {
        name: p.name,
        code: p.code,
        lat: p.lat,
    ...
    // Prepare chart totals
    const systemTotals = { Rooftop: 0, Farm: 0, Floating: 0, Carpark: 0, BESS: 0 };
    projects.forEach(p => {
      if (p.systems) {
        Object.entries(p.systems).forEach(([sysName, cap]) => {
          if (systemTotals[sysName] !== undefined) {
            systemTotals[sysName] += cap;
          }
        });
      }
    });

    const chartLabels = ['Rooftop', 'Farm', 'Floating', 'Carpark', 'BESS [Battery Storage]'];
...
<head>
  ...
  <style>
    ...
    .clean-modern-table td {
      border-bottom: 1px solid var(--card-border) !important;
      color: var(--text-color);
    }
    .clean-modern-table tr:hover {
      background-color: rgba(2, 87, 37, 0.03);
    }
...
<body>
  ...
  <div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
      <div>
        <h1 class="text-success mb-1" style="font-weight: 800; font-size: 28px;">${title}</h1>
        <p class="text-muted mb-0" style="font-size: 14px;">${subtitle} l Generated on ${new Date().toLocaleString()}</p>
      </div>
    </div>
    ...
    <div class="row g-4 align-items-stretch mb-5">
      ...
      <div class="col-lg-8">
        <div class="glass-card p-4 h-100 d-flex flex-column">
          <h5 class="mb-3 text-success-emphasis" style="font-weight: 700;"><i class="fas fa-map-marked-alt text-success me-2"></i>Project Coordinates on Thailand Map</h5>
      ...
      <div class="col-lg-4 d-flex flex-column justify-content-between">
        <!-- Doughnut Chart -->
        <div class="glass-card p-4 mb-4 flex-grow-1 d-flex flex-column">
          <h5 class="mb-3 text-success-emphasis" style="font-weight: 700;"><i class="fas fa-chart-pie text-success me-2"></i>Portfolio (MWp)</h5>
          <div class="d-flex flex-column flex-grow-1 align-items-center justify-content-between">
            <div style="position: relative; height: 260px; width: 100%; max-width: 260px; margin: 0 auto 15px auto;">
              <canvas id="doughnutChart"></canvas>
            </div>
            <div id="system-details" style="width: 100%;">
              ${detailsHtml}
            </div>
          </div>
        </div>
        ...
        <div class="glass-card p-4 flex-grow-1 d-flex flex-column">
          <h5 class="mb-3 text-success-emphasis" style="font-weight: 700;"><i class="fas fa-table text-success me-2"></i>Solar Capacity by Region (MWp)</h5>
          <div class="table-responsive">
            <table class="table table-sm text-center align-middle mb-0 clean-modern-table" style="font-size: 11px; width: 100%;">
              <thead>
                <tr class="border-bottom border-success-subtle fw-bold" style="color: var(--primary-color);">
                  <th class="text-start ps-2">System Type</th>
                  <th>Central</th>
                  <th>North</th>
                  <th>South</th>
                  <th>Northeast</th>
  ...
  <script>
    ...
    function getPinIconSvg(color) {
      const colorVal = encodeURIComponent(color);
      return "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' width='36' height='36'><path fill='" + colorVal + "' d='M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z'/></svg>";
    }

    markers.forEach(m => {
      const icon = L.icon({
        iconUrl: getPinIconSvg(pinColor),
        iconSize: [36, 36],
        iconAnchor: [18, 36],
        popupAnchor: [0, -32]
      });
    ...
    new Chart(ctx, {
      ...
      plugins: [{
        ...
        beforeDraw: function(chart) {
          ...
          const valueText = totalVal.toFixed(2) + " MWp";
          const valFontSize = Math.round(innerRadius * 0.22);
          ctx.font = "bold " + valFontSize + "px Outfit, sans-serif";
          ctx.fillStyle = isDark ? "#e2ece6" : "#0d1a12";
          const valueX = Math.round((width - ctx.measureText(valueText).width) / 2);
          const valueY = height / 2 - 14;
          ...
          const labelText = "TOTAL";
          const labelFontSize = Math.round(innerRadius * 0.14);
          ctx.font = "bold " + labelFontSize + "px Inter, sans-serif";
          ctx.fillStyle = isDark ? "#8ca395" : "#536b5c";
          const labelX = Math.round((width - ctx.measureText(labelText).width) / 2);
          const labelY = height / 2 + 6;
          ...
          const projCountText = projectsCount + " " + (projectsCount === 1 ? 'Project' : 'Projects');
          const projFontSize = Math.round(innerRadius * 0.13);
          ctx.font = "bold " + projFontSize + "px Inter, sans-serif";
          ctx.fillStyle = isDark ? "#8ca395" : "#536b5c";
          const projX = Math.round((width - ctx.measureText(projCountText).width) / 2);
          const projY = height / 2 + 22;
